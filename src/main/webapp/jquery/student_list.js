    ...
    $("#updateForm").validate({ // 定义验证规则
        ...
        submitHandler: function (form) {    // 当前表单对象
           ...
           var sid = $("#ssid").text();
            ...
            $.post("pages/back/student/student_update.action",{"sid":sid,"name":name,"age":age,"address":address,"sex":sex,"cid":cid},function(obj){
                if(obj.trim() == "true"){
...
function loadData() {   // 定义数据读取的操作函数
    $.post("pages/back/student/student_list.action", {"cp": jsCommonCp, "ls": jsCommonLs}, function (obj) {
        ...
        for(var x=0;x < obj.allStudent.length;x++){
            ...
            addRow(obj.allStudent[x].sid,obj.allStudent[x].name,obj.allStudent[x].age,sex,obj.allStudent[x].address,obj.allStudent[x].classes);
        }
...
function loadClasses(cid) {
	$.post("pages/back/classes/classes_list.action",{},function (obj) {
        ...
        for (var x = 0;x <obj.allClasses.length;x++){
            	$('#cid').append( '<option value="'+ obj.allClasses[x].cid +'">'+obj.allClasses[x].cname+'</option>' );
